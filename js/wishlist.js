document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateWishlistCount = () => {
        ...
        // Update the wishlist count display in the header section
        const wishlistCountDisplay = document.getElementById('wishlist-count-display');
        if (wishlistCountDisplay) {
            wishlistCountDisplay.textContent = wishlistItems.length;
        }
        
        // Show/hide clear all button based on wishlist items
        const clearAllBtn = document.getElementById('clear-all-btn');
        if (clearAllBtn) {
            clearAllBtn.style.display = wishlistItems.length > 0 ? 'inline-block' : 'none';
        }
    };
    ...
    // Add event listener for header 'Continue Shopping' button
    const continueShoppingHeaderBtn = document.getElementById('continue-shopping-header-btn');
    if (continueShoppingHeaderBtn) {
        continueShoppingHeaderBtn.addEventListener('click', () => {
            ...
            if (confirm('Are you sure you want to remove all items from your wishlist?')) {
                try {
                    ...
                    for (const item of wishlistItems) {
                        await apiCall(`/wishlist/${item.book._id}`, { method: 'DELETE' });
                    }
